# model code for sideb model for malishev m & civitello dj biocontrol   
...
# choose either algae (rg_pars) or detritus (detr_pars)   
# rg_pars <- 0.25
# detr_pars <- 0
# snail_snack_min_vec <- c(0) # min size host to eat
# snail_snack_max_vec <- c(0,5,10,15,25,50) # max size host to eat
...
# detr_pars <- c(0,0.01,0.05,0.1,0.2,0.25) # detritus input (mg L^-1 day^-1)
# snail_snack_min_vec <- 0 # min size host to eat
# snail_snack_max_vec <- 0 # max size host to eat

# pred_ps sims on drive 26-6-20
pred_ps_total <- c(0,0.01,0.05,seq(0.1,1.5,0.1),2,2.5,3,3.5,4,4.5,5,10,15)

# pred density param for sims
# discrete host mortality 
pred_ps <- c(seq(0,1.5,0.1),2)  # fig 1&2
pred_ps <- c(seq(0,1.5,0.1),2,2.5,3) # fig 3
# continuous host mortality with varying resource productivity   
pred_ps <- c(seq(0, 0.2, 0.05), 0.5, 1, 1.5, 2, 3, 5) # fig 4 

# per m^2
# discrete host mortality 
c(seq(0,1.5,0.1),2) / 50 * 500 # fig 1&2
...
# for @productivity == 1 # plus need to comment out rg and detr in next loop    
            ...
            for(me_event in me_events){ # 1:7 # run all me event scenarios and save to file.
              ...
              if(snail_control==1){    
                if(hailmary==1){ # @hailmary
                  fhh = paste0(resource_type,"_",me_fh,"_rep",rn);fhh #
                  # fhh = paste0(resource_type,"_",me_fh,"_meim",me_im_event,"_rep",rn);fhh # use for hailmary120 for me_im_event = 1:5. 
              ...
              ####################################  start netlogo sim ######################################## 
              for(hb in hb_pars){
                for(detr in detr_pars){ # loop through detritus inputs # comment out for @productivity
                  for(alpha in alpha_pars){ # loop through alphas (amplitude in food cycle)
                    for(rho in rho_pars){ # loop through rhos (periodicity of food cycle)
                      for(rg in rg_pars){ # loop through rgs (food growth rates) # comment out for @productivity
